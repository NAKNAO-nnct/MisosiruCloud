...
class ZoneStore extends Controller
{
    public function __construct(private readonly DnsZoneRepository $dnsZoneRepository)
    {
    }

    public function __invoke(SaveDnsZoneRequest $request): RedirectResponse
    {
        $data = $request->validated();
        $data['is_active'] = (bool) ($data['is_active'] ?? false);

        $this->dnsZoneRepository->create($data);

        return redirect()->route('dns-zones.index')->with('success', 'DNSゾーンを作成しました。');
    }
}
