...
| dns | coredns/coredns | 53 (TCP/UDP) | 内部 DNS サーバ |
...
[テナント VM / サービス]
    ...
    ▼
[S3 プロキシ (s3-proxy.internal:9000)]
...
クラスタ内で稼働する **CoreDNS** による内部 DNS サーバ。
各 VM やコンテナは内部 DNS を参照することで、内部サービスへのアクセスをローカルネットワーク経由にルーティングする。

### 6.5.2 目的

- **コンテナレジストリの近道:** グローバルドメインで公開しているレジストリを、クラスタ内からアクセスする際にローカル IP に解決し、トラフィックの遠回り (グローバル経由) を防ぐ
- **内部サービスの名前解決:** `s3-proxy.internal`, `mgmt-app.internal` 等の内部ドメインを解決
- **サービスディスカバリ:** 将来的なサービス間通信のための名前解決基盤
...
| リッスンIP | 172.26.26.x:53 (管理NW), 172.26.27.x:53 (VM NW) |
| 内部ゾーン | `.internal` ゾーン (静的レコード) |
| スプリットホライズン | 外部ドメインを内部IPに上書き解決 |
| フォワーダ | 非内部ドメインは 8.8.8.8 / 8.8.4.4 に転送 |

### 6.5.4 CoreDNS 設定 (Corefile)
...
**internal.hosts:**
```
172.26.26.10  mgmt-app.internal
172.26.26.10  s3-proxy.internal
172.26.26.10  dns.internal
172.26.26.10  registry.internal
172.26.26.10  otel-gateway.internal
172.26.26.11  snippet-api-pve1.internal
172.26.26.12  snippet-api-pve2.internal
172.26.26.13  snippet-api-pve3.internal
```

**override.hosts (スプリットホライズン例):**
```
# コンテナレジストリ: グローバルドメインでも内部IPに解決
172.26.26.10  registry.example.com
```

> **ポイント:** `registry.example.com` がグローバルでも使われるドメインの場合、
> クラスタ内部から参照すると `172.26.26.10` に解決され、VPS やインターネットを経由せずに
> 直接ローカルの Harbor にアクセスできる。
...
resolv_conf:
  nameservers:
    - 172.26.26.10    # 内部 DNS サーバ (dns.internal)
  searchdomains:
    - internal
  options:
    edns0: true
...
> **効果:** VM 起動時に `/etc/resolv.conf` が内部 DNS を参照するよう自動設定される。
> `s3-proxy.internal`, `registry.internal` 等の内部ドメインが解決可能になる。
| nomad-worker | Nomad Worker | base + Docker + Nomad agent + cAdvisor |
