...
- [x] `compose.prod.yaml` 作成（初期版、Harbor は未統合）
...
- [x] `.env.prod.example` — 本番用環境変数テンプレート
- [x] Docker ネットワーク定義
...
- [x] `queue` サービスを追加（Phase 11 で定義した Queue Worker）
- [x] `dns` サービスを追加（開発用 CoreDNS — 任意）
...
- [x] `docker/dns/Corefile` 作成（初期テンプレート — ローカルゾーンなし）
  ```
  ...
  ```
- [x] `docker/dns/zones/` ディレクトリ作成（ゾーンファイル格納先）
- [x] `docker/dns/Dockerfile` 作成（CoreDNS 公式イメージベース）
- [x] Docker Compose ボリュームマウント設定
...
- [x] `App\Models\DnsZone` — Eloquent Model
- [x] `App\Models\DnsRecord` — Eloquent Model
- [x] `App\Data\Dns\DnsZoneData` — `final readonly class` (of/make/toArray)
- [x] `App\Data\Dns\DnsRecordData` — `final readonly class` (of/make/toArray)

#### Repository

- [x] `App\Repositories\DnsZoneRepository` — findByIdOrFail, findByProvider, all, create, update, delete
- [x] `App\Repositories\DnsRecordRepository` — findByZoneId, create, update, delete, findByZoneIdAndType

#### Lib\Dns (プロバイダ層)

- [x] `App\Lib\Dns\DnsProviderInterface` — listRecords, createRecord, updateRecord, deleteRecord
- [x] `App\Lib\Dns\CloudflareDnsProvider` — Cloudflare API v4 実装
...
- [x] `App\Lib\Dns\SakuraDnsProvider` — さくらのクラウド DNS API 実装
  - HTTP クライアント: さくら API v2
  - 認証: Access Token + Access Token Secret (config 経由)
- [x] `App\Lib\Dns\LocalDnsProvider` — CoreDNS ゾーンファイル生成
...
- [x] `App\Lib\Dns\DnsProviderFactory` — provider 文字列 → インスタンス解決

#### Service

- [x] `App\Services\DnsService` 作成
...
- [x] `Admin\Dns\ZoneIndex` — ゾーン一覧画面（ゾーン名・プロバイダ・レコード数）
- [x] `Admin\Dns\ZoneStore` — ゾーン追加
- [x] `Admin\Dns\ZoneUpdate` — ゾーン更新
- [x] `Admin\Dns\ZoneDestroy` — ゾーン削除
- [x] `Admin\Dns\RecordIndex` — レコード一覧画面（ゾーン配下のレコード表示）
- [x] `Admin\Dns\RecordStore` — レコード追加 (+ SaveDnsRecordDataRequest FormRequest)
- [x] `Admin\Dns\RecordUpdate` — レコード更新
- [x] `Admin\Dns\RecordDestroy` — レコード削除
- [x] `Admin\Dns\ZoneSync` — 外部プロバイダからレコード同期
- [x] `Admin\Dns\Reload` — CoreDNS リロード
- [x] View: `resources/views/admin/dns/zone-index.blade.php`
- [x] View: `resources/views/admin/dns/record-index.blade.php`
...
- [x] Harbor のデプロイ方式決定（Docker Compose or Helm）
- [x] `docker/registry/` ディレクトリ作成
- [x] Harbor 設定ファイル (`harbor.yml`) のテンプレート作成
...
#### Laravel テスト
- [x] Unit: `DnsZoneData` / `DnsRecordData` の `of()` / `make()` / `toArray()` 変換テスト
- [x] Unit: `DnsZoneRepository` / `DnsRecordRepository` の CRUD テスト
- [x] Unit: `LocalDnsProvider` のゾーンファイル生成が RFC 1035 準拠であること
- [x] Unit: `LocalDnsProvider` の Corefile 生成が正しいドメインブロックを含むこと
- [x] Feature: `CloudflareDnsProvider` の API 呼び出し (Http::fake)
- [x] Feature: `SakuraDnsProvider` の API 呼び出し (Http::fake)
- [x] Feature: `DnsService::createRecord()` が DB 保存 + Provider 反映を行うこと
- [x] Feature: `DnsService::regenerateLocalZones()` がゾーンファイルを生成すること
- [x] Feature: DNS 管理画面のゾーン一覧・レコード CRUD が動作すること
