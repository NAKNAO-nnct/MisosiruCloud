...
| ip_address | VARCHAR(45) | YES | NULL | テナント VNet 内の固定 IP (例: 10.1.0.10) |
| gateway | VARCHAR(45) | YES | NULL | デフォルトゲートウェイ (例: 10.1.0.1) |
| vnet_name | VARCHAR(64) | YES | NULL | Proxmox SDN VNet 名 (例: vnet_1) |
| shared_ip_address | VARCHAR(45) | YES | NULL | VM Network IP (172.26.27.x, Dual NIC時) |
| ssh_keys | TEXT | YES | NULL | SSH 公開鍵 (複数行可) |
| provisioning_status | ENUM('pending','uploading','cloning','configuring','starting','ready','error') | NO | 'pending' | プロビジョニング状態 |
...
**注記:** VMのCPU・メモリ・ディスク・状態等の情報は Proxmox API から都度取得する。`ip_address` / `gateway` / `vnet_name` は Cloud-init ネットワーク設定および NIC 設定に使用する。`shared_ip_address` が NULL の場合はテナント VNet のみの Single NIC 構成。
